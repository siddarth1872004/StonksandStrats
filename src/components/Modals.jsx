...
const SPECIAL_INFO = {
  go: { title: "GO", color: "#34d399", text: "Collect $200 salary as you pass or land here. The starting square of the board." },
  tax: { title: null, color: "#f87171", text: null }, // handled inline (uses tile.price)
  chance: { title: "CHANCE", color: "#fbbf24", text: "Draw a Chance card — movement, cash swings, repairs, or a Get Out of Jail Free card. Higher risk, higher reward." },
  community_chest: { title: "COMMUNITY CHEST", color: "#38bdf8", text: "Draw a Community Chest card — usually bank payouts, fees, or a Get Out of Jail Free card." },
  jail: { title: "JAIL / JUST VISITING", color: "#F59E0B", text: "Just visiting? No effect. Sent here? Roll doubles, pay $50, or use a Get Out of Jail Free card to leave." },
  free_parking: { title: "FREE PARKING", color: "#fbbf24", text: "A safe resting square. No rent, no fee — unless a house rule puts a cash pot here." },
  go_to_jail: { title: "GO TO JAIL", color: "#ef4444", text: "Go directly to Jail. Do not pass GO, do not collect $200." },
};

// Compute the rent that would currently be owed if an opponent landed here.
...
export function PropertyDetailModal({ tileId, gameState, onClose }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4" onClick={onClose} style={{ zIndex: 8000 }}>
      <div
      ...
      >
        ...
        {/* LIVE rent due — the most useful at-a-glance number */}
        {liveRent && (
